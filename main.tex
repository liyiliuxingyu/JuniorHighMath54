...
\usepackage{zwfh}
\allowdisplaybreaks
\ctexset{section={name={第,节},number=\arabic{section},},subsection={name={第,课时},number=\arabic{subsection},}}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \tkzLabelSegment[right=.2cm,pos=.5](A,C){$b$}
    \end{tikzpicture}
    \caption{三角形}
    \label{fig:triangle}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{等腰三角形}
    \label{fig:isosceles_triangle(IT,等腰三角形)}
    \end{minipage}
\end{figure}
\par {\heiti 直角三角形的两个锐角互余.}
\subsection{}
\par 有两边相等的三角形叫做等腰三角形，如图\ref{fig:isosceles_triangle(IT,等腰三角形)}.
